...
template <>
bool ParserSingleton<ParserDraw>::ParseElement(
    const tinyxml2::XMLElement* element, ParserStatus* status) {
  auto node = std::make_shared<LayoutDraw>();
  ...
  element->QueryIntAttribute("vertexCount", &node->vertex_count);
  element->QueryIntAttribute("instanceCount", &node->instance_count);
  element->QueryIntAttribute("firstVertex", &node->first_vertex);
