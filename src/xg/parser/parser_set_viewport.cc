...
template <>
bool ParserSingleton<ParserSetViewport>::ParseElement(
    const tinyxml2::XMLElement* element, ParserStatus* status) {
  auto node = std::make_shared<LayoutSetViewport>();
  ...
  element->QueryIntAttribute("firstViewport", &node->first_viewport);
