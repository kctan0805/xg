...
std::shared_ptr<xg::Layout> Application::CreateLayout(int index) const {
  auto layout = xg::Parser::Get().ParseFile(
      (index % 2 == 0)
          ? "../../../app/change_layout/layouts/change_layout0.xml"
          : "../../../app/change_layout/layouts/change_layout1.xml");
  return layout;
}
...
xg::Result Application::OnUpdate(std::shared_ptr<xg::Viewer> viewer) {
  ...
  *uniform_data = camera->GetViewMatrix();
  ...
  if (--draw_count_ <= 0) return xg::Result::kTimeout;
